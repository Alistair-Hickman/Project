...
def main_loop(x_pos,y_pos,z_con,frame):
 ...
 im.save("/Users/alistair/Documents/Project/Scripts/Frames.nosync/Frame_" + str(frame) + ".png")
 return 0
...
for frame in range(no_frames):
    for ii in range(no_bugs):
     ...
     x_pos.append(int((x_vals[ii][frame]*ppm)+512))
     y_pos.append(int((y_vals[ii][frame]*ppm)+512))
     z_con.append((p*z_vals[ii][frame]*z_vals[ii][frame])+q)
    ...
    if __name__ == '__main__':
      proc = mp.Process(target=main_loop(x_pos,y_pos,z_con,frame))
      proc.start()

    #data_array = main_loop(x_pos,y_pos,z_con)
